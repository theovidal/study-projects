...
def une_iteration(B,V,omega):
    ecartmax=0
    for i in range(Nx):
        for j in range(Ny):
            pot=V[i,j]
            if B[i,j]==0:
                V[i,j]= (1-omega)*V[i,j] + omega *(V[i+1,j]+V[i-1,j]+V[i,j+1]+V[i,j-1])/4.
                ecartmax=max(ecartmax,abs(pot-V[i,j]))
    return(ecartmax)

def iterations(B,V,eps,omega):
    ecart = eps+1.
    nb_iterations = 0
    while ecart>eps:
        ecart=une_iteration(B,V,omega)
        nb_iterations+= 1
